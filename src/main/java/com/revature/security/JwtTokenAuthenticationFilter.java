...
/**
 * A filter used to intercept all requests and validate the JWT, if present, in the HTTP request
 * header.
 */
public class JwtTokenAuthenticationFilter extends OncePerRequestFilter {
  ...
  /**
   * Constructor for JwtTokenAuthenticationFilter that instantiates the JwtConfig and ZuulConfig
   * fields.
   *
   ...
   * valid only if it has the proper prefix, a proper principal, and is unexpired.
   *
  ...
  public void doFilterInternal(HttpServletRequest req, HttpServletResponse resp, FilterChain chain)
    ...
    if (header == null || !header.startsWith(jwtConfig.getPrefix())) {
      ...
      ctx.addZuulRequestHeader(
          zuulConfig.getHeader(),
          get_SHA_512_SecureHash(zuulConfig.getSalt(), zuulConfig.getSecret()));
    ...
    try {
      ...
      if (username != null) {
        ...
        // 7. Add the Zuul header to validate that requests received by Auth and Project services
        // came through Zuul
        RequestContext ctx = RequestContext.getCurrentContext();
        ctx.addZuulRequestHeader(
            zuulConfig.getHeader(),
            get_SHA_512_SecureHash(zuulConfig.getSecret(), zuulConfig.getSalt()));
      }
  ...
  /**
   * This is a function to wrap a static method so that we can test it easier. @Author Alonzo Muncy
   * (190107 Java-Spark-USF)
   *
